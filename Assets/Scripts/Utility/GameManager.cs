...
public class GameManager : MonoBehaviour
{
    ...
    [Tooltip("Whether the game can be won by surviving a certain amount of time")]
    public bool canWinBySurviving = true;
    [Tooltip("The time in seconds the player needs to survive to win the game")]
    public float survivalTime = 30f;
    public float remainingSurvivalTime;
    ...
    /// <summary>
    /// Description:
    /// Coroutine that tracks the survival time and triggers level cleared if the player survives for the required time
    /// Inputs:
    /// none
    /// Returns:
    /// IEnumerator
    ...
    {
        while (remainingSurvivalTime > 0)
        {
            yield return new WaitForSeconds(1f);
            remainingSurvivalTime--;
        }
    ...
    private IEnumerator UpdateSurvivalTimeUI()
    {
        while (remainingSurvivalTime > 0 && !gameIsOver)
        {
            UpdateUIElements();
            yield return new WaitForSeconds(1f);
        }
